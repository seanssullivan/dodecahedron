...
# Validators
# ----------------------------------------------------------------------------
def is_final_release(version: Any, /) -> bool:
    ...
    if not isinstance(version, Version):
        message = f"expected type 'Version', got {type(version)} instead"
        raise TypeError(message)

    result = not version.dev and not version.pre
    return result
